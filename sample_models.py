...
def cnn_rnn_model(input_dim, filters, kernel_size, conv_stride,
    ...
    # TODO: Add batch normalization
    bn_rnn = BatchNormalization(name='bn_conv_1d_2')(simp_rnn)
    # TODO: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
    y_pred = Activation('softmax', name='softmax')(time_dense)
    ...
    for i in range(recur_layers):
        if i == 0:
            rnn = GRU(units, activation='relu',
                      return_sequences=True, name=f'hidden_{i + 1}')(input_data)
            bn_rnn = BatchNormalization(name=f'bn_{i + 1}')(rnn)
        else:
            rnn = GRU(units, activation='relu',
                      return_sequences=True, name=f'hidden_{i + 1}')(bn_rnn)
            bn_rnn = BatchNormalization(name=f'bn_{i + 1}')(rnn)
    # Dense layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(lstm)(input_data)
    # TODO: Add batch normalization
